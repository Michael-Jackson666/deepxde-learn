...
Brusselator是一个经典的反应扩散系统，描述了两种化学物质$u$和$v$的时空演化：
...
| $D_u$ | 0.2 | $u$的扩散系数 |
| $D_v$ | 0.1 | $v$的扩散系数 |
| `domain_size` | 1.0 | 空间域大小 $[0, \text{domain\_size}]^2$ |
| `time_end` | 2.0 | 时间域终点 $[0, \text{time\_end}]$ |

## 📊 使用技巧

### 1. 网络设计要点
- **多输出结构**: 输出维度=2 (对应$u$和$v$)
...
### 3. 参数调优
- **$A, B$参数**: 控制反应动力学，不同值产生不同的时空模式
- **扩散系数**: $D_u$和$D_v$的比值影响空间模式的形成
...
当$D_v > D_u$时，系统可能产生图灵不稳定性，形成空间周期性模式。
